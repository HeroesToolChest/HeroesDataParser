...
namespace HeroesData.Parser.XmlData
{
    public class TalentData : AbilityTalentData
    {
        ...
        public Talent? CreateTalent(Hero hero, XElement talentArrayElement)
        {
            ...
            ArgumentNullException.ThrowIfNull(talentArrayElement);
        ...
        private void SetAbilityTalentLinkIds(Hero hero, Talent talent, XElement talentElement)
        {
            ...
            if (talent.AbilityTalentId.AbilityType == AbilityTypes.Heroic)
            {
                ...
                {
                    XElement? behaviorArrayElement = rankArrayElement
                        .Elements("BehaviorArray")?
                        .FirstOrDefault(x =>
                            !string.IsNullOrEmpty(x.Attribute("value")?.Value) &&
                            x.Attribute("value")!.Value.StartsWith("Ultimate", StringComparison.OrdinalIgnoreCase) &&
                            x.Attribute("value")!.Value.EndsWith("Unlocked", StringComparison.OrdinalIgnoreCase));
        ...
        private void ValidatorPlayerTalentCheck(string? validatorId, string abilityTalentId)
        {
            ...
            if (validatorPlayerTalentElement != null)
            {
                ...
                if (_abilityTalentIdsByTalentIdUpgrade.TryGetValue(talentReferenceNameId, out HashSet<string>? value))
                    value.Add(abilityTalentId);
                else
        ...
        {
            ArgumentNullException.ThrowIfNull(talentElement);
            ArgumentNullException.ThrowIfNull(talent);
            ArgumentNullException.ThrowIfNull(hero);
            ...
            foreach (XElement element in talentElement.Elements())
            {
                ...
                else if (elementName == "TRAIT")
                {
                    setTraitAction = () =>
                    {
                        if ((talent.IsActive is true && (talent.AbilityTalentId.AbilityType == AbilityTypes.Unknown)) ||
                            (talent.IsActive is false && (talent.AbilityTalentId.AbilityType == AbilityTypes.Unknown || talent.AbilityTalentId.AbilityType == AbilityTypes.Hidden)))
                        {
                    ...
                    setAbilAction = () =>
                    {
                        ...
                        if (!string.IsNullOrEmpty(abilValue))
                        {
                            ...
                            if (abilityElements.Any())
                            {
                                ...
                                {
                                    if (talent.IsActive || talent.AbilityTalentId.AbilityType == AbilityTypes.Unknown || talent.AbilityTalentId.AbilityType == AbilityTypes.Hidden)
