    ...
    internal class MountDataXmlWriter : MountDataWriter<XElement, XElement>
    {
        ...
        protected override XElement MainElement(Mount mount)
        {
            ...
            return new XElement(
                ...
                mount.VariationMountIds.Count > 0 ? new XElement("VariationMounts", mount.VariationMountIds.OrderBy(x => x, StringComparer.OrdinalIgnoreCase).Select(d => new XElement("VariationMount", d))) : null!);
        }
    }
}
