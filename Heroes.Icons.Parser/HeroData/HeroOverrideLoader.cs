...
namespace Heroes.Icons.Parser.HeroData
{
    public class HeroOverrideLoader
    {
        ...
        public string HeroDataOverrideXmlFile => @"HeroData\HeroOverrides.xml";
        ...
        public Dictionary<string, HashSet<string>> AdditionalHeroUnitsByCUnit { get; set; } = new Dictionary<string, HashSet<string>>();
            ...
            foreach (var hero in cHeroes)
            {
                ...
                foreach (var dataElement in hero.Elements())
                {
                    ...
                    else if (elementName == "Ability")
                    {
                        ...
                        var redirectElement = dataElement.Elements("Redirect").FirstOrDefault();
                    ...
                    {
                        AddAdditionalUnits(dataElement.Attribute("value")?.Value, cHeroId, dataElement);
                    }
                }
            }
        }
        ...
        private void AddAdditionalUnits(string type, string cHeroId, XElement dataElement)
        {
            if (type == "Hero")
            {
                foreach (XElement unitElement in dataElement.Elements("Unit"))
                {
                    ...
                    {
                        if (AdditionalHeroUnitsByCUnit.TryGetValue(cHeroId, out HashSet<string> currentUnits))
                        {
                            if (!currentUnits.Contains(unit))
                            {
                                currentUnits.Add(unit);
                                AdditionalHeroUnitsByCUnit[cHeroId] = currentUnits;
                            }
                        }
                        else
                        {
                            AdditionalHeroUnitsByCUnit.Add(cHeroId, new HashSet<string> { unit });
                        }
                    }
                }
            }
